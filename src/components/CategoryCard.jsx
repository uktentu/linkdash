...
import { Plus, MoreHorizontal, Folder, FolderOpen, Edit2, Trash2, Briefcase, User, GripVertical, Pin } from 'lucide-react';
...
export function CategoryCard({ category, viewMode, onEdit, onDelete, onAddUrl, onEditUrl,
    ...
    // const [isCollapsed, setIsCollapsed] = useState(false); // Removed in favor of persistent state
    ...
    const {
        ...
        transition,
        isDragging
    } = useSortable({ id: category.id, disabled: isReadOnly || category.pinned });
    ...
    const style = {
        ...
        WebkitColumnBreakInside: 'avoid-column',
    };
        ...
        <div ref={setNodeRef} style={style}>
            ...
            >
                ...
                <div className="relative p-5 pl-12 flex items-center justify-between border-b border-[var(--border-subtle)]/50">
                    {/* Drag Handle or Pin Indicator */}
                    {!isReadOnly && (
                        category.pinned ? (
                            <div className="absolute left-3 top-1/2 -translate-y-1/2 p-1.5 text-amber-400">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="w-10 h-10 rounded-xl bg-[var(--bg-input)] flex items-center justify-center text-[var(--text-secondary)] border border-[var(--border-subtle)] shadow-sm hover:bg-[var(--bg-card-hover)] transition-colors cursor-pointer"
                            title={category.isCollapsed ? "Expand" : "Collapse"}
                        >
                            {category.isCollapsed ? (
                                <Folder className="w-5 h-5 transition-transform duration-300" />
                            ) : (
                                <FolderOpen className="w-5 h-5 transition-transform duration-300" />
                            )}
                        </button>
                        <div>
                                ...
                                <AnimatePresence>
                                    {isMenuOpen && (
                                        ...
                                        >
                                            ...
                                            <button
                                                onClick={() => { setIsMenuOpen(false); onTogglePin && onTogglePin(); }}
                                                className={`w-full px-4 py-2.5 text-left text-sm flex items-center gap-2 ${category.pinned ? 'text-amber-400 hover:bg-amber-500/10' : 'text-[var(--text-secondary)] hover:bg-[var(--bg-card-hover)] hover:text-[var(--text-primary)]'}`}
                                            >
                                                <Pin className={`w-4 h-4 ${category.pinned ? 'fill-current' : ''}`} /> {category.pinned ? 'Unpin Collection' : 'Pin Collection'}
                                            </button>
                                            <button
